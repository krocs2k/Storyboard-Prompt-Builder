  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-slate-900 border border-amber-500/30 rounded-2xl w-full max-w-4xl h-[calc(100vh-1.5rem)] max-h-[850px] overflow-hidden flex flex-col"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-2.5 border-b border-slate-700/80 shrink-0">
          <h2 className="text-lg font-bold text-amber-400">Upload Screenplay</h2>
        ...
        <div className="flex-1 flex flex-col min-h-0 px-5 py-3">
          {error && (
            <div className="mb-3 px-3 py-2.5 bg-red-500/20 border border-red-500/50 rounded-lg text-red-400 flex items-center gap-2 text-sm shrink-0">
          ...
          <AnimatePresence mode="wait">
            {!analysis ? (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="flex flex-col gap-4 flex-1 min-h-0 justify-center"
              >
                {/* Progress Indicator */}
                {loading && (
                  <div className="shrink-0">
                            ...
                            <div className="flex flex-col items-center">
                              <motion.div
                                ...
                                className={`w-8 h-8 rounded-full flex items-center justify-center ${
                                  isActive ? 'ring-3 ring-amber-500/30' : ''
                                }`}
                              >
                                {isComplete ? (
                              ...
                              <span className={`text-[10px] mt-1 ${isActive || isComplete ? 'text-amber-400' : 'text-slate-500'}`}>
                                {step.label}
                              </span>
                            </div>
                            {index < PROGRESS_STEPS.length - 1 && (
                              <div className={`w-10 h-0.5 mx-1.5 ${isComplete ? 'bg-amber-500' : 'bg-slate-700'}`} />
                            )}
                          </div>
                        );
                      })}
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-slate-500 text-xs mt-0.5">Please wait while we analyze your screenplay</p>
                    </div>
                  </div>
                )}
                ...
                {!loading && (
                  <>
                    ...
                    >
                      <input ref={fileInputRef} type="file" accept=".txt,.md,.doc,.docx"
                      ...
                      {file ? (
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          <p className="text-slate-500 text-[10px] mt-3">Supports .txt, .md, .doc, .docx</p>
                        </div>
                      )}
                    </div>

                    <button onClick={analyzeScreenplay} disabled={!file || loading}
                      ...
                      <Sparkles className="w-4 h-4" /> Analyze Screenplay
                    </button>
                  </>
                )}
              </motion.div>
              ...
              >
                {/* Analysis Summary — compact inline badges */}
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                  <span className="text-slate-400 text-xs font-medium">Analysis:</span>
                  <span className="px-2.5 py-1 bg-slate-800 rounded-md text-white text-xs font-medium">{analysis.title || 'Untitled'}</span>
                  <span className="px-2.5 py-1 bg-slate-800 rounded-md text-slate-300 text-xs">{analysis.estimatedRuntime || '?'} min</span>
                  <span className="px-2.5 py-1 bg-slate-800 rounded-md text-slate-300 text-xs">{analysis.genre || 'Unknown'}</span>
                  <span className="px-2.5 py-1 bg-slate-800 rounded-md text-slate-300 text-xs">{analysis.mood || 'Unknown'}</span>
                </div>

                {/* Visual Recommendations */}
                <div className="bg-slate-800/40 rounded-xl p-4 flex-1 min-h-0 flex flex-col">
                  <div className="flex items-center justify-between mb-3 shrink-0">
                    <h3 className="text-sm font-semibold text-white">Visual Recommendations</h3>
                    <button onClick={applyAllRecommendations}
                      className="px-3 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/50 rounded-lg text-amber-400 text-xs transition-colors">
                  ...
                  <div className="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      ...
                      {analysis.recommendations.imageType && (
                        <CompactRecRow section={1} icon={<Palette className="w-3.5 h-3.5" />} label="Image Type"
                          value={analysis.recommendations.imageType.recommended} reason={analysis.recommendations.imageType.reason}
                      ...
                      {analysis.recommendations.lighting && analysis.recommendations.lighting[0] && (
                        <CompactRecRow section={3} icon={<Sun className="w-3.5 h-3.5" />} label="Lighting"
                          value={analysis.recommendations.lighting[0].type} reason={analysis.recommendations.lighting[0].reason}
                      ...
                      {analysis.recommendations.cameraBody && (
                        <CompactRecRow section={4} icon={<Camera className="w-3.5 h-3.5" />} label="Camera"
                          value={analysis.recommendations.cameraBody.recommended} reason={analysis.recommendations.cameraBody.reason}
                      ...
                      {analysis.recommendations.focalLength && (
                        <CompactRecRow section={4} icon={<Camera className="w-3.5 h-3.5" />} label="Focal Length"
                          value={analysis.recommendations.focalLength.recommended} reason={analysis.recommendations.focalLength.reason}
                      ...
                      {analysis.recommendations.lensType && (
                        <CompactRecRow section={4} icon={<Camera className="w-3.5 h-3.5" />} label="Lens Type"
                          value={analysis.recommendations.lensType.recommended} reason={analysis.recommendations.lensType.reason}
                          onApply={() => applyRecommendation('lensType', analysis.recommendations.lensType!.recommended)} />
                      )}
                      {analysis.recommendations.filmStock && (
                        <CompactRecRow section={4} icon={<Film className="w-3.5 h-3.5" />} label="Film Stock"
                          value={analysis.recommendations.filmStock.recommended} reason={analysis.recommendations.filmStock.reason}
                          onApply={() => applyRecommendation('filmStock', analysis.recommendations.filmStock!.recommended)} />
                      )}
                      {analysis.recommendations.aspectRatio && (
                        <CompactRecRow section={4} icon={<Film className="w-3.5 h-3.5" />} label="Aspect Ratio"
                          value={analysis.recommendations.aspectRatio.recommended} reason={analysis.recommendations.aspectRatio.reason}
                      ...
                      {analysis.recommendations.photographerStyle && (
                        <CompactRecRow section={5} icon={<Sparkles className="w-3.5 h-3.5" />} label="Photographer"
                          value={analysis.recommendations.photographerStyle.recommended} reason={analysis.recommendations.photographerStyle.reason}
                      ...
                      {analysis.recommendations.movieStyle && (
                        <CompactRecRow section={5} icon={<Film className="w-3.5 h-3.5" />} label="Movie Style"
                          value={analysis.recommendations.movieStyle.recommended} reason={analysis.recommendations.movieStyle.reason}
                      ...
                      {analysis.recommendations.filterEffect && (
                        <CompactRecRow section={5} icon={<Sparkles className="w-3.5 h-3.5" />} label="Filter"
                          value={analysis.recommendations.filterEffect.recommended} reason={analysis.recommendations.filterEffect.reason}
                ...
                <button onClick={onClose}
                  className="w-full py-3 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-900 font-semibold rounded-xl transition-all text-sm shrink-0">
...
function CompactRecRow({ section, icon, label, value, reason, onApply }: {
  section: number; icon: React.ReactNode; label: string; value: string; reason: string; onApply: () => void;
}) {
  return (
    <div className="flex items-center gap-2.5 px-3 py-2 bg-slate-900/60 rounded-lg">
      <div className="w-5 h-5 rounded-full bg-amber-500/20 flex items-center justify-center text-amber-400 text-[10px] font-bold shrink-0">{section}</div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-slate-600 text-[10px] truncate">{reason}</p>
      </div>
      <button onClick={onApply}
        className="px-2 py-1 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/40 rounded text-amber-400 text-[10px] font-medium transition-colors shrink-0">
        Apply
      </button>
    </div>
  );
