...
import { Eye, EyeOff, Loader2, Mail, Lock, User, Film, CheckCircle2 } from 'lucide-react';
...
function RegisterContent() {
  ...
  useEffect(() => {
    setMounted(true);
    fetch('/api/admin/users/signup-setting')
      .then(res => res.json())
      .then(data => setSignupDisabled(data.signupDisabled === true))
      .catch(() => {})
  ...
  if (!mounted || status === 'loading' || checkingSignup) {
    return (
        ...
        <div className="w-full max-w-md">
          <div className="bg-gray-800 border border-gray-700 rounded-2xl shadow-xl p-8 text-center">
            ...
            <p className="text-gray-400 mb-6">
              Public registration is currently disabled. If you need access, please contact an administrator to receive an invitation.
            </p>
            <Link
              href="/login"
...
const DynamicRegisterContent = dynamic(() => Promise.resolve(RegisterContent), { 
  ssr: false,
  loading: () => (
    <div className="min-h-screen flex items-center justify-center ">
      <Loader2 className="w-8 h-8 animate-spin text-rose-500" />
