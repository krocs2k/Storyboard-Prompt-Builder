...
function buildPersonaSystemPrompt(personas?: PersonaInfo[]): string {
  if (!personas || personas.length === 0) {
    return `You are a creative screenwriter and story developer. Generate unique, compelling story ideas that would make excellent screenplays. Each idea should be specific enough to develop into a full story but open enough for creative interpretation.`;
  }
...
Respond with raw JSON only. Do not include code blocks, markdown, or any other formatting.`
    ...
    trackUsage({ eventType: 'story_idea', apiModel: llm.model, apiType: 'llm' });
